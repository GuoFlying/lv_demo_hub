...
static void lv_100ask_calc_error(lv_100ask_error_t error_code ,lv_100ask_error_t err);
...
// error list
static const lv_100ask_error_table_t lv_100ask_error_table[] = {
    ...
    if(code == LV_EVENT_VALUE_CHANGED)
    {
        ...
        if (strcmp(txt, "=") == 0)
        {
            ...
            if (calc->error_code != no_error)
            {
                ...
                for (int i = 0; i < sizeof(lv_100ask_error_table); i++)
                {
                    if (lv_100ask_error_table[i].error_code == calc->error_code)
                    {
                        lv_textarea_add_text(calc->ta_hist, "\n");
                        lv_textarea_add_text(calc->ta_hist, lv_100ask_error_table[i].message);
                        lv_textarea_add_text(calc->ta_hist, "\n");
                    }
                }
                calc->error_code = no_error;
