...
LV_IMG_DECLARE(app_img_lv_demo_music_btn_list_play);
LV_IMG_DECLARE(app_img_lv_demo_music_btn_list_pause);
...
lv_obj_t *_app_music_list_create(lv_obj_t *parent)
{
    ...
    uint32_t track_id;
    for (track_id = 0; _app_music_get_title(track_id); track_id++)
    {
    ...
    _app_music_list_btn_check(0, true);
...
void _app_music_list_btn_check(uint32_t track_id, bool state)
{
    ...
    if (state)
    {
        ...
        lv_img_set_src(icon, &app_img_lv_demo_music_btn_list_pause);
        lv_obj_scroll_to_view(btn, LV_ANIM_ON);
    }
    else
    {
        lv_obj_clear_state(btn, LV_STATE_CHECKED);
        lv_img_set_src(icon, &app_img_lv_demo_music_btn_list_play);
    }
}
...
static lv_obj_t *add_list_btn(lv_obj_t *parent, uint32_t track_id)
{
    uint32_t t = _app_music_get_track_length(track_id);
    char time[32];
    lv_snprintf(time, sizeof(time), "%" LV_PRIu32 ":%02" LV_PRIu32, t / 60, t % 60);
    const char *title = _app_music_get_title(track_id);
    const char *artist = _app_music_get_artist(track_id);
    ...
    LV_IMG_DECLARE(app_img_lv_demo_music_list_border);
    lv_obj_t *border = lv_img_create(btn);
    lv_img_set_src(border, &app_img_lv_demo_music_list_border);
