 ...
 * @LastEditors: Flying
 * @LastEditTime: 2022-05-01 18:47:43
 * @Description: 新建文件
 */
#include "../app.h"
...
static void event_cb(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_DRAW_PART_END)
    {
        ...
        if (dsc->id == 3)
        {
            LV_IMG_DECLARE(app_img_star);
            lv_img_header_t header;
            lv_res_t res = lv_img_decoder_get_info(&app_img_star, &header);
            if (res != LV_RES_OK)
                return;
            ...
            lv_draw_img(dsc->draw_ctx, &img_draw_dsc, &a, &app_img_star);
        }
    }
}
